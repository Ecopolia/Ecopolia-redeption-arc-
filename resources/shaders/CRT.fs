...
extern MY_HIGHP_OR_MEDIUMP number transition_amount; // New uniform for transition effect
...
vec4 effect(vec4 color, Image tex, vec2 tc, vec2 pc)
{
    ...
    MY_HIGHP_OR_MEDIUMP float distance_from_center = length(orig_tc - screen_center);
    ...
    vec4 transition_col = vec4(rgb_result * (1.0 - circle_mask), 1.0);

    return (transition_col * (1. - 1. * bloom) + bloom * col) * mask;
}
